...
int process_image(const char* input_path, const char* output_path, int width, int height, float clip_limit) {
    ...
    int size = pixel_count * 4;  // ARGB: 4 bytes per pixel for both input and output
    ...
    for (int i = 0, j = 0; i < pixel_count; i++, j += 4) {
        // BGRA: B, G, R, A
        img.b[i] = input_data[j];
        img.g[i] = input_data[j + 1];
        img.r[i] = input_data[j + 2];
        img.a[i] = input_data[j + 3];
        // ARGB: A, R, G, B
        // img.a[i] = input_data[j];     
        // img.r[i] = input_data[j + 1]; 
        // img.g[i] = input_data[j + 2]; 
        // img.b[i] = input_data[j + 3]; 
    }
    ...
    // Combine channels into output buffer including alpha
    ...
    for (int i = 0, j = 0; i < pixel_count; i++, j += 4) {
        // ARGB: R, G, B, A
        output_data[j] = out_r[i];
        output_data[j + 1] = out_g[i];
        output_data[j + 2] = out_b[i];
        output_data[j + 3] = img.a[i];
    }
